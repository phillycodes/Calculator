...
int main() {
	...
	// Menu selection
	printf("Calulator Menu\n\nPlease enter one the letter in the parenthesis below:\n");
	printf("(A)dd\n(S)ubtract\n(M)ultiply\n(D)ivide\nA(V)erage\n(G)reater Value\n(P)ositive, Negative, or Zero\n(Q)uit\n");
	scanf_s(" %c", &menuSelect);

	// After selection switch statement will follow the correct case
	switch (menuSelect) {
	case 'a':
	case 'A':
		result = num1 + num2;
	...
	case 'q':
	case 'Q':
		printf("You have chose to quit. ");
		break;
	default:
		printf("You have an invalid input. Please restart the program. Good-bye!\n");
		break;
	}
	printf("Good-Bye!");
	return 0;
